...
% gviz_write_class(+Stream, +Instances, +TypeName):-
%   Write all objects (Instances) of a given class of name TypeName.

gviz_write_class(Stream, Instances, TypeName) :-
   (foreach(Object, Instances),
    param(Stream, TypeName)
    do
      gviz_write_object(Stream, Object, TypeName)
   ).    
...
%   TODO: Access to fields in superclasses

gviz_write_object(Stream, Object, TypeName) :-
  % The identifier used within the graphviz file will be   
  % type name in lowr case + oid of the object:
  getOid(Object, Oid),
  printf(Stream, "%s%d [shape=record,label=\"{%s%d: %s |", 
         [TypeName, Oid, TypeName, Oid, TypeName]),
...
gviz_write_binary_link(Stream, Link, TypeName) :-
   ...
   printf(Stream, "%s%d -- %s%d [label=\"%s\"];", 
         [RoleType1, Oid1, RoleType2, Oid2, TypeName]),
   writeln(Stream, "").     
